...
public class ContentRepositorySupport {
    ...
    public Content findByIdContentResponse(Long id) {
    ...
    public List<Content> findFollowContent(List<Long> userIds) {
        return queryFactory
                .selectFrom(
                        content1
                ).where(
                        content1.user.id.in(userIds), content1.createdDate.between(LocalDateTime.now().minusDays(4), LocalDateTime.now())
                ).fetch();
    }

    public List<ContentLike> findAlarmContentLike(Long id) {
        return queryFactory
                .selectFrom(contentLike)
                .where(contentLike.content.user.id.eq(id), contentLike.createdDate.between(LocalDateTime.now().minusDays(4), LocalDateTime.now()))
                .fetch();
    }
    public Page<ClotheInfoSearchResponse> searchInfo(Pageable pageable, String query) {
        QueryResults<ClotheInfoSearchResponse> result =
                queryFactory
                        .select(Projections.constructor(ClotheInfoSearchResponse.class, clothesInfo, clothesInfo.brandName))
                        .from(clothesInfo)
                        .where(clothesInfo.id.isNotNull(), queryInfo(query))
                        .limit(pageable.getPageSize())
                        .offset(pageable.getOffset())
                        .fetchResults();
        return new PageImpl<>(result.getResults(), pageable, result.getTotal());
    }
    private Predicate queryInfo(String query) {
        if (query == null) {
            return null;
        }
        return clothesInfo.brandName.contains(query).or(clothesInfo.name.contains(query));
    }
}
